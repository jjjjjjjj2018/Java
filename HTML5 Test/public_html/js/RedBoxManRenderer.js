...
function renderShapesRedBoxMan(location) {
    ...
    gc.fillRect(location.x + 15 + 12.5, location.y + 13 + 10, pupilS, pupilS);
    gc.beginPath();

    //right eye
    gc.fillStyle = eyeColor;
    gc.fillRect(location.x + 15 + 30 + 25, location.y + 13, eyeW, eyeH);
    gc.beginPath();

    gc.strokeStyle = outlineColor;
    gc.lineWidth = 1;
    gc.rect(location.x + 15 + 30 + 25, location.y + 13, eyeW, eyeH);
    gc.stroke();

    gc.fillStyle= pupilColor;
    gc.fillRect(location.x + 15 + 12.5 + 30 + 25, location.y + 13 + 10, pupilS, pupilS);
    gc.beginPath();

    //mouth
    gc.fillStyle = mouthColor;
    gc.fillRect(location.x + 20, location.y + 66, mouthW, mouthH);
    gc.beginPath();

    //body
    gc.fillStyle = bodyColor;
    gc.fillRect(location.x + 32, location.y + 88, bodyW, bodyH);
    gc.beginPath();

    //hands
    gc.fillStyle = handColor;
    gc.fillRect(location.x + 23, location.y + 88, handW, handH);
    gc.beginPath();

    gc.fillStyle = handColor;
    gc.fillRect(location.x + 82, location.y + 88, handW, handH);
    gc.beginPath();

    //feet
    gc.fillStyle = footColor;
    gc.fillRect(location.x + 25.5, location.y + 88 + 30, footS, footS);
    gc.beginPath();

    gc.fillStyle = mouthColor;
    gc.fillRect(location.x + 78, location.y + 88 + 30, footS, footS);
    gc.beginPath();
}
