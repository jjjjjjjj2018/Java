...
public class RandomClusterer extends Clusterer {
    ...
    public RandomClusterer(ApplicationTemplate applicationTemplate, DataSet dataset, int maxIterations, int updateInterval, int numberOfClusters, boolean countRun) {
    ...
    public void run() {
        ...
        if (countRun()) {
            isRunning = true;
            for (int i = 1; i <= maxIterations; i += updateInterval) {//& tocontinue.get()
                assignLabels();
                recomputeCentroids();
                Platform.runLater(() -> {
                    chart.getData().clear();
        ...
        dataset.getLocations().forEach((instanceName, location) -> {
            ...
            for (int i = 0; i < centroids.size(); i++) {
                ...
                if (distance < minDistance) {
                    ...
                    int  n = rand.nextInt(numberOfClusters) + 1;
                    minDistanceIndex = n;
                }
            }
